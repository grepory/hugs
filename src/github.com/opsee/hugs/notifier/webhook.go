...
type FullCheckResponse struct {
	Target   *checker.Target  `protobuf:"bytes,1,opt,name=target" json:"target,omitempty"`
...
type FullCheckResult struct {
	...
	Timestamp  *checker.Timestamp   `protobuf:"bytes,3,opt,name=timestamp" json:"timestamp,omitempty"`
	Passing    bool                 `protobuf:"varint,4,opt,name=passing" json:"passing,omitempty"`
	Responses  []*FullCheckResponse `protobuf:"bytes,5,rep,name=responses" json:"responses,omitempty"`
	Target     *checker.Target      `protobuf:"bytes,6,opt,name=target" json:"target,omitempty"`
...
func NewFullCheckResult(checkResult *checker.CheckResult) (*FullCheckResult, error) {
	...
	for _, response := range checkResult.Responses {
		...
		// Unmarshal protobuf
		any, err := checker.UnmarshalAny(response.Response)
		if err != nil {
			log.Error(err)
			return nil, err
		}
		...
		httpResponse, ok := any.(*checker.HttpResponse)
		if !ok {
